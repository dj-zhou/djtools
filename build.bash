...
function _build_meson_use_oesdk() { # sdk_path
    ...
    if [ -n "$2" ] && [ $2 = '--fresh' ]; then
        ...
        if [ -d $sdk_output ]; then
            ...
            cd $sdk_output
            ninja
            echo -e "${PRP}build meson -cross${NOC}"
...
function compile_makefile() {
    echo -e "use ${PRP}Makefile${NOC} to build"
    clean_tag=$1
    if [ "$clean_tag" = "clean" ]; then
        echo -e "${PRP} make clean${NOC}"
        make clean
        return
    fi
    echo -e "${PRP}make -j$(nproc) $clean_tag${NOC}"
...
function compile_cmakelist() {
    echo -e "use ${PRP}CMakeLists.txt${NOC} to build/clean"
    cur_dir=${PWD}
    build_dir="_bnative.cmake"
    clean_tag=$1
    if [[ -z "$clean_tag" || "$clean_tag" = "all" ]]; then
        if [ ! -d "$build_dir" ]; then
            echo -e "${PRP}mkdir "$build_dir" && cd "$build_dir" && cmake ..${NOC}\n"
...
function build() {
    ...
    echo -e "${PRP}build${NOC}: argument ${RED}$1${NOC} not supported."
    return
}
